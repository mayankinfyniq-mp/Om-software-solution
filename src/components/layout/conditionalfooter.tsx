    ...
    <footer className="relative overflow-hidden border-t border-white/5">
      {!isContactPage && (
        <>
          ...
          <div
            className="overflow-hidden border-y border-white/5 py-8 md:py-10"
            aria-hidden
          >
      ...
      <div className="grid gap-12 px-5 py-16 md:grid-cols-[1.5fr_1fr_1fr_1.3fr] md:px-10 md:py-20">
        <div>
          ...
          <p className="mt-4 max-w-xs text-sm leading-relaxed text-accent/55">
            A full-stack software studio crafting cinematic digital products
            from Ahmedabad, India — for the world.
          </p>
          ...
          <ul className="mt-5 space-y-3 text-sm">
            {navLinks.map((l) => (
              <li key={l.href}>
                <Link
                  href={l.href}
                  className="link-line text-accent/70 transition-colors hover:text-accent"
                >
                  {l.label}
                </Link>
              </li>
      ...
      <div className="flex flex-col items-center justify-between gap-4 border-t border-white/5 px-5 py-6 text-xs text-accent/45 md:flex-row md:px-10">
        ...
        <p className="flex items-center gap-2">
          Crafted with <span className="text-primary">🧡</span> in Ahmedabad,
          India
        </p>
